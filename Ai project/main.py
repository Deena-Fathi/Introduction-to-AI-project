...
#In the main repository we make a directory/folder where it will hold all the atendee's photos
# We have multiple images, importing them individually can become messy.
# Therefore we will write a script to import all images in a given folder at once.
# For this we will need the os library so we will import that first.
...
# Now that we have a list of images we can iterate through those and create a corresponding encoded list for known faces.
...
# we have to create a video capture object so that we can grab frames from the webcam.
...
# Even though the image being used is 1/4 th of the original,
# we will still use the original size while displaying.
...
# Lastly we are going to add the automated attendance code.
...
# Next we can split using comma ‘,’. This will allow us to get the first element which is the name of the user.
